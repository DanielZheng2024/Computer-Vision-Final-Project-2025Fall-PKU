...
class StableDiffusion(nn.Module):
    ...
    def get_image_embeddings(self, image: torch.Tensor) -> torch.Tensor:
        ...
        with torch.no_grad():
            ...
            image_embeddings = self.clip_vision_model(**inputs).image_embeds
    ...
    def train_step(self, 
                   ...
                   latent_coef=0,
                   image_coef=0,
                   # 新增两个参数用于Image Prompt 
                   ref_image:Optional[torch.Tensor]=None, 
                   ref_image_coef:float=0, 
                  ):
        ...
        with torch.no_grad():
            ...
            if image_coef or ref_image_coef: 
        ...
        # 新增：Image Prompt 引导梯度
        if ref_image is not None and ref_image_coef > 0:
            # 需要参考图是512 * 512 在notebook里需要对参考图预处理
            ref_image_512 = F.interpolate(ref_image, (512, 512), mode='bilinear').to(self.device)
            image_prompt_delta = pred_rgb_512 - ref_image_512 # 像素级残差
            pred_rgb_512.backward(gradient = w * image_prompt_delta * ref_image_coef, retain_graph=True)
        ...
        grad = w * (noise_delta * noise_coef + latent_delta * latent_coef)
